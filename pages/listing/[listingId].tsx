...
function ListingPage({}: Props) {
  ...
  const nftBoughtSuccess = () => toast("NFT Bought Successfully!");
  const bidSuccess = () => toast("Bid made successfully!");
  const offerSuccess = () => toast("Offer Made Successfully!");

  const [minnimumNextBid, setMinimumNextBid] = useState<{
    displayValue: string;
    symbol: string;
  }>();
  ...
  const formatPlaceholder = () => {
    ...
    if (listing.type === ListingType.Auction) {
      return Number(minnimumNextBid?.displayValue) === 0
        ? "Enter Bid Amount"
        : `${minnimumNextBid?.displayValue} ${minnimumNextBid?.symbol} or more`;
        ...
        <section className="flex-1 space-y-5 pb-20 lg:pb-0">
          ...
          <div className="grid grid-cols-2 space-y-2 items-center justify-end">
            ...
            {listing.type === ListingType.Auction && (
              <>
                <p>Current Minnimum Bid:</p>
                <p className="font-bold">
                  {minnimumNextBid?.displayValue} {minnimumNextBid?.symbol}
                </p>
